...
parser = argparse.ArgumentParser(
        description="Run a test file against a L4u implementation")
...
parser.add_argument('--pre-eval', default=None, type=str,
        help="L4u code to evaluate prior to running the test")
...
parser.add_argument('test_file', type=str,
        help="a test file formatted as with l4u test data")
parser.add_argument('l4u_cmd', nargs="*",
        help="L4u implementation command line. Use '--' to "
             "specify a L4u command line with dashed options.")
...
if sys.argv.count('--') > 0:
    args.l4u_cmd = sys.argv[sys.argv.index('--')+1:]
...
r = Runner(args.l4u_cmd, no_pty=args.no_pty, line_break="\r\n" if args.crlf else "\n")
...
while t.next():
    ...
    # https://github.com/kanaka/l4u/issues/30
...
add_result(timestamp+results.replace("\n", ","))
