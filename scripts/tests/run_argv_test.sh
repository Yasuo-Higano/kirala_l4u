...
#
# Usage: run_argv_test.sh <command line arguments to run l4u>
#
...
if [ -z "$1" ] ; then
  echo "Usage: $0 <command line arguments to run l4u>"
  exit 1
fi

root="$(dirname $0)"

out="$( $@ $root/print_argv.l4u aaa bbb ccc | tr -d '\r' )"
...
#
# out="$( $@ $root/print_argv.l4u aaa 'bbb ccc' ddd )"
# assert_equal '("aaa" "bbb ccc" "ddd")' "$out"

out="$( $@ $root/print_argv.l4u | tr -d '\r' )"
assert_equal '()' "$out"
