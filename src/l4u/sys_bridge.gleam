...
import l4u/l4u_type.{type Expr, type PDic, type Ref}
...
@external(javascript, "../l4u_sys_bridge_ffi.mjs", "erl_sym")
...
pub fn catch_dummy(try_fn: fn() -> any) -> Result(any, err) {
  Ok(try_fn())
}
...
@target(javascript)
@external(javascript, "../l4u_sys_bridge_ffi.mjs", "compare_any_")
fn compare_any_(x: any, y: any) -> Int

@target(javascript)
pub fn compare_any(x: Expr, y: Expr) -> Order {
  case compare_any_(l4u_to_native(x), l4u_to_native(y)) {
    0 -> order.Eq
    n if n < 0 -> order.Lt
    _ -> order.Gt
  }
